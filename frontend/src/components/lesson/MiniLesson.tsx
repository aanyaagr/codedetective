...
import { completeLesson, getCase, type CaseSummary } from "@/lib/api";
...
export default function MiniLesson() {
  ...
  useEffect(() => {
    getCase(CASE_ID)
      .then(({ case: data, progress }) => {
        setCaseData(data);
        setCompleted(progress?.completedStages?.includes("LEARN") ?? false);
      })
    ...
    <main className="lesson-page">
      ...
      <section className="lesson-content">
        ...
        <div className="lesson-clue">
          <div className="clue-heading"><Lightbulb size={19} /> CLUE</div>
          <div className="clue-content"><p>{lesson.clue}</p><p>CASE CONCEPT: <span className="red-text">{caseData.concept.toUpperCase()}</span></p><p>OBJECTIVE: {caseData.objective}</p></div>
          <div className="clue-decoration"><div className="clue-folder"><div className="clue-magnifier" /></div></div>
        </div>

        <div className="lesson-section-title"><span /><h2>KEY IDEAS</h2><span /></div>
        <div className="modifier-grid">
          {lesson.sections.map((section) => <div className="modifier-card" key={section.title}><div className="modifier-icon"><BookOpen size={42} /></div><h3>{section.title}</h3><div className="modifier-divider" /><p>{section.description}</p></div>)}
        </div>
        ...
        <div className="lesson-navigation">
          <Link href="/evidence" className="lesson-back-button"><ArrowLeft size={20} /> BACK TO EVIDENCE TRAIL</Link>
          <button onClick={finishLesson} className="lesson-continue-button">{completed ? "LESSON COMPLETED" : "COMPLETE LESSON"} <ArrowRight size={21} /></button>
        </div>
      </section>
    </main>
  );
